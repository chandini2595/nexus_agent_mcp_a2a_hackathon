...
class TemporalTaskManager(InMemoryTaskManager):
    """Temporal Task Manager, handles workflow task routing and response packing."""

    def __init__(self, agent: TemporalAgent):
        super().__init__()
        self.agent = agent
    ...
    async def on_send_task(
        ...
        if not utils.are_modalities_compatible(
            ...
            TemporalAgent.SUPPORTED_CONTENT_TYPES,
        ):
            logger.warning(
    ...
    async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        if result is not None:
            ...
            parts = [{"type": "text", "text": "couldn't execute Temporal workflow agent"}]
