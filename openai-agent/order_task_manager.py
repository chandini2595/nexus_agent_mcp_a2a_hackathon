...
class OrderTaskManager(InMemoryTaskManager):
    """Order Task Manager, handles task routing and response packing."""

    def __init__(self, agent: OrderAgent):
        super().__init__()
        self.agent = agent
    ...
    async def on_send_task(
        ...
        if not utils.are_modalities_compatible(
            ...
            OrderAgent.SUPPORTED_CONTENT_TYPES,
        ):
            logger.warning(
    ...
    async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        if result is not None:
            ...
            parts = [{"type": "text", "text": "couldn't execute Order agent"}]
