...
class JiraTaskManager(InMemoryTaskManager):
    """Jira Task Manager, handles task routing and response packing."""

    def __init__(self, agent: JiraAgent):
        super().__init__()
        self.agent = agent
    ...
    async def on_send_task(
        ...
        if not utils.are_modalities_compatible(
            ...
            JiraAgent.SUPPORTED_CONTENT_TYPES,
        ):
            logger.warning(
    ...
    async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        if result is not None:
            ...
            parts = [{"type": "text", "text": "couldn't execute Jira agent"}]
