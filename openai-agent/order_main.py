"""Main entry point for the Order Status Agent.
...
from order_task_manager import OrderTaskManager
from starlette.applications import Starlette
...
async def lifespan(app: Starlette) -> AsyncIterator[State]:
    ...
    This function starts the MCP server with the order_status.py script locally and closes it once the starlette server shuts down
    """
...
class OrderA2AServer:
    ...
    def _get_agent_card(self, request: Request) -> JSONResponse:
        ...
        self.task_manager = OrderTaskManager(agent=OrderAgent(mcp_server))
...
def main(host, port):
    """Entry point for the A2A + Order Status Agent."""
    ...
    try:
        ...
        skill = AgentSkill(
            id="order_status_tracker",
            name="Order Status Tracker",
            description=(
                "Check status of orders, list all orders, and update order status"
            ),
            tags=["Order Status", "Track Order", "Update Order"],
            examples=[
        ...
        agent_card = AgentCard(
            ...
            description="Track and manage order status information",
            url=f"http://{host}:{port}/",
            version="1.0.0",
            defaultInputModes=OrderAgent.SUPPORTED_CONTENT_TYPES,
            defaultOutputModes=OrderAgent.SUPPORTED_CONTENT_TYPES,
            capabilities=capabilities,
            skills=[skill],
        )

        server = OrderA2AServer(
            agent_card=agent_card, task_manager=OrderTaskManager(agent=OrderAgent())
        )

        logger.info(f"Starting Order Status server on {host}:{port}")
        server.start()
        print("Order Status server started...")
